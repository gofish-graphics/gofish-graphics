 ...
 * light/dark mode.
 */
withDefaults(defineProps<{ cols?: number | string }>(), { cols: 3 });
</script>

<template>
  <div class="tutorial-grid" :style="{ '--tutorial-cols': String(cols) }">
    <slot />
  </div>
</template>
...
.tutorial-grid {
  display: grid;
  grid-template-columns: repeat(var(--tutorial-cols), minmax(0, 1fr));
  gap: 16px;
  margin: 20px 0 32px;
}
