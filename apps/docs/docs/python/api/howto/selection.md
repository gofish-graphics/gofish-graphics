...
the bare field name: `group(by="species")`, not `group(by="datum.species")`.
...
`None` and you must disaggregate first. See
...
the one the default infers, name it with `along` — see the
[`ribbon` docs](/python/api/marks/ribbon#default-grouping) for the full rule.
See [`.layer()`](/python/api/core/layer).
