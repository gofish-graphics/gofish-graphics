...
PNG). Polar wedge fidelity is now high; point-node radial layouts are still maturing —
see the polar gaps section.
...
Point-node (circle) radial layouts are still limited by the gaps listed below; each story
carries a `// NOTES:` block documenting its specifics. `copy/` (a stray duplicate of the
sunburst family) is skipped.
...
`radial-deep` via per-leaf data-position box packing (#627).
...
1. **Angular auto-fit: wedges DONE, point nodes still open.** Filled WEDGE (rect) nodes get real angular auto-fit: tag them `thetaSize: datum(1)` and the coord (now a fit-frame on θ) allocates angle by subtree leaf weight and fits the 2π budget (#622) — this is what closes the sunburst/sector/clock/tyre rings. POINT nodes (circles) still get a fixed constant θ-spacing (angle is not allocated from node count), so radial node-link trees (RadialTree, RadialTreeIncline, RadialPhylogeneticTree, …) are still hand-radian and overflow/wrap on wide trees. `radial-deep` demonstrates the workaround: compute per-leaf slot positions in the story's data pass and emit raw radians/px (#627). Point marks also hit a center-offset under the coord transform (#678).
